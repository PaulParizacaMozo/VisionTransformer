...
Tensor Conv2d::forward(const Tensor &input, bool isTraining)
{
    ...
    output = contiguous_cuda(output);
    // output = output.contiguous();
    // if (verify(output, cuda_out, 1e-5f) == false)
    // {
    ...
    // if (verify(output, output_cuda, 1e-5f) == false)
    // {
    //     std::cerr << "Error en la verificación de addBroadcast 4d\n";
    // }
    return output;
}
...
Tensor Conv2d::backward(const Tensor &outputGradient)
{
    ...
    // if (verify(reshaped_out_grad, cuda_reshaped, 1e-5f) == false)
    // {
    //     std::cerr << "Error en la verificación de contiguous 3d\n";
    // }
    ...
    // im2col_transposed = im2col_transposed.contiguous();
    // if (verify(im2col_transposed, cuda_im2col, 1e-5f) == false)
    // {
    ...
    // weights_transposed = weights_transposed.contiguous();
    // if (verify(weights_transposed, cuda_weights, 1e-5f) == false)
    // {
    //     std::cerr << "Error en la verificación de contiguous 2d\n";
    // }
