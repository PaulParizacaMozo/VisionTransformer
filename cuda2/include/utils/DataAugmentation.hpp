        ...
        for (size_t b = 0; b < image.getShape()[0]; ++b)
        {
            for (size_t c = 0; c < image.getShape()[1]; ++c)
            {
                for (size_t h = 0; h < height; ++h)
                {
                    ...
                    {
                        auto value = image.get({b, c, h, width - w - 1});
                        flipped.set({b, c, h, w}, value);
        ...
        for (size_t c = 0; c < image.getShape()[1]; ++c)
        {
            for (size_t h = 0; h < height; ++h)
            {
                for (size_t w = 0; w < width; ++w)
                {
                    ...
                    {
                        auto valueu = image.get({b, c, (size_t)y_new, (size_t)x_new});
                        rotated_image.set({b, c, h, w}, valueu);
                        // rotated_image(b, c, h, w) = image(b, c, (size_t)y_new, (size_t)x_new);
                    }
                    else
                    {
                        rotated_image.set({b, c, h, w}, 0.0f);
        ...
        for (size_t c = 0; c < image.getShape()[1]; ++c)
        {
            for (size_t h = 0; h < height; ++h)
            {
                for (size_t w = 0; w < width; ++w)
                {
                    ...
                    {
                        auto vsds = image.get({b, c, h, w});
                        translated_image.set({b, c, static_cast<size_t>(new_h), static_cast<size_t>(new_w)}, vsds);
        ...
        for (size_t c = 0; c < image.getShape()[1]; ++c)
        {
            for (size_t h = 0; h < new_height; ++h)
            {
                for (size_t w = 0; w < new_width; ++w)
                {
                    ...
                    {
                        auto vadada = image.get({b, c, static_cast<size_t>(orig_h), static_cast<size_t>(orig_w)});
                        zoomed_image.set({b, c, h + pad_top, w + pad_left}, vadada);
