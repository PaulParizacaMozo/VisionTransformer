...
namespace
{
  ...
  float calculate_accuracy(const Tensor &logits, const Tensor &labels)
  {
    ...
    for (size_t i = 0; i < batch_size; ++i)
    {
      ...
      if (labels.get({i, static_cast<size_t>(pred_class)}) == 1.0f)
      {
        correct_predictions++;
      }
...
std::pair<float, float> Trainer::train_epoch(const Tensor &X_train, const Tensor &y_train, int epoch)
{
  ...
  for (size_t i = 0; i < num_batches; ++i)
  {
    ...
    for (size_t j = 0; j < count; ++j)
    {
      ...
      for (size_t c = 0; c < channels; ++c)
      {
        for (size_t h = 0; h < height; ++h)
        {
          for (size_t w = 0; w < width; ++w)
          {
            ...
            auto value = x_sample.get({0, c, h, w});
            X_batch.set({j, c, h, w}, value);
      ...
      {
        auto value = y_sample.get({0, c});
        y_batch.set({j, c}, value);
