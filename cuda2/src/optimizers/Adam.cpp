...
//       }
//       else
//       { // Para embeddings posicionales, etc. (3D)
...
void Adam::update(std::vector<Tensor *> &parameters, const std::vector<Tensor *> &gradients)
{
  ...
  for (size_t i = 0; i < parameters.size(); ++i)
  {
    ...
    if (shape.size() == 1 || shape.size() == 2 || shape.size() == 3)
    {
      if (shape.size() == 1)
      {
        param->adamUpdate1D(*grad_tensor, m_i, v_i, learningRate, beta1, beta2, beta1_t, beta2_t, epsilon, weight_decay);
      }
      else if (shape.size() == 2)
      { // Para pesos de Dense
        param->adamUpdate2D(*grad_tensor, m_i, v_i, learningRate, beta1, beta2, beta1_t, beta2_t, epsilon, weight_decay);
      }
      else
      { // Para embeddings posicionales, etc. (3D)
        param->adamUpdate3D(*grad_tensor, m_i, v_i, learningRate, beta1, beta2, beta1_t, beta2_t, epsilon, weight_decay);
      }
    }
  }
}
